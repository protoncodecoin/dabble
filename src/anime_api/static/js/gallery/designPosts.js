...
const postAnimationContainerEl = document.querySelector("#gallery_design");
...
const getPosts = async (offset, limit) => {
  const API_Endpoint = `/content/designcontent/?limit=${limit}&offset=${offset}`;

  const response = await fetch(API_Endpoint);

  if (!response.ok)
    throw new Error(
      `An error occurred from design content: ${response.statusText}`
    );
