const postVideoContainerEl = document.querySelector("#gallery_video");
  ...
  if (!emptyAnimationPage && !blockAnimationRequest) {
    ...
    if (results.length === 0) {
      ...
      const htmlData = results
        ...
        .join("");
      postVideoContainerEl.insertAdjacentHTML("beforeend", htmlData);
    }
...
const getPosts = async (offset, limit) => {
  const API_Endpoint = `/content/videocontent/?limit=${limit}&offset=${offset}`;

  const response = await fetch(API_Endpoint);

  if (!response.ok)
    throw new Error(
      `An error occurred from video content: ${response.statusText}`
    );
...
postVideoContainerEl.addEventListener("click", function (e) {
  if (e.target && e.target.matches("page-content-card")) {
